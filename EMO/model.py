...
from basic_modules import get_norm, get_act, ConvNormAct, MSPatchEmb
...
class iRMB(nn.Module):
    def __init__(self, dim_in, dim_out, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
                 act_layer='relu', act_layer2='silu', v_proj=True, dw_ks=3, stride=1, dilation=1, se_ratio=0.0,
                 dim_head=64, window_size=7,
        ...
        self.conv_local = ConvNormAct(dim_mid, dim_mid, kernel_size=dw_ks, stride=stride, dilation=dilation,
                                      groups=dim_mid,
                                      norm_layer='bn_2d', act_layer=act_layer2, inplace=inplace)
    ...
    def forward(self, x):
        ...
        if self.attn_s:
            ...
            b, c, h, w = x.shape
            qk = self.qk(x)
            # qk = qk.reshape(2, B * n1 * n2, self.num_head, h * w, self.dim_head).contiguous()  # since CoreML does
            # not support reshaping operator greater than 5
...
class EMO(nn.Module):
    def __init__(self, dim_in=3, num_classes=1000, stem_dim=24,
                 depths=None, embed_dims=None, exp_ratios=None,
                 dim_heads=None, act_layers=None, act_layer2='silu',
                 norm_layers=('bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'),
                 dw_kss=(3, 3, 5, 5),
                 se_ratios=(0.0, 0.0, 0.0, 0.0),
                 window_sizes=(7, 7, 7, 7),
                 attn_ss=(False, False, True, True),
                 qkv_bias=True, attn_drop=0., drop=0., drop_path=0.5,
                 v_group=False, attn_pre=True, pre_dim=0):
        super().__init__()
        if depths is None:
            depths = [1, 2, 4, 2]
        ...
        self.stage0 = nn.ModuleList([
            ...
            iRMB(  # ds
                ...
                norm_layer=norm_layers[0], act_layer=act_layers[0], act_layer2=act_layer2,
        ...
        for i in range(len(depths)):
            ...
            for j in range(depths[i]):
                ...
                layers.append(iRMB(
                    ...
                    norm_layer=norm_layers[i], act_layer=act_layers[i], act_layer2=act_layer2,
...
default_act_layer = ['silu', 'silu', 'gelu', 'gelu']


def EMO_1M(num_classes=1000, act_layer='silu', **kwargs):
    act_layers = [act_layer for _ in range(4)] if act_layer != 'silu' else default_act_layer

    model = EMO(
        num_classes=num_classes,
        depths=[2, 2, 8, 3], embed_dims=[32, 48, 80, 168], exp_ratios=[2., 2.5, 3.0, 3.5],
        dim_heads=[16, 16, 20, 21], drop_path=0.04036, act_layers=act_layers, act_layer2=act_layer,
        **kwargs)
    return model


def EMO_2M(num_classes=1000, act_layer='silu', **kwargs):
    act_layers = [act_layer for _ in range(4)] if act_layer != 'silu' else default_act_layer

    model = EMO(
        num_classes=num_classes,
        depths=[3, 3, 9, 3], embed_dims=[32, 48, 120, 200], exp_ratios=[2., 2.5, 3.0, 3.5],
        dim_heads=[16, 16, 20, 20], act_layers=act_layers, act_layer2=act_layer,
        **kwargs)
    return model


def EMO_5M(num_classes=1000, act_layer='silu', **kwargs):
    act_layers = [act_layer for _ in range(4)] if act_layer != 'silu' else default_act_layer

    model = EMO(
        num_classes=num_classes,
        depths=[3, 3, 9, 3], embed_dims=[48, 72, 160, 288], exp_ratios=[2., 3., 4., 4.],
        dim_heads=[24, 24, 32, 32], act_layers=act_layers, act_layer2=act_layer,
        **kwargs)
    return model


def EMO_6M(num_classes=1000, act_layer='silu', **kwargs):
    act_layers = [act_layer for _ in range(4)] if act_layer != 'silu' else default_act_layer

    model = EMO(
        num_classes=num_classes,
        depths=[3, 3, 9, 3], embed_dims=[48, 72, 160, 320], exp_ratios=[2., 3., 4., 5.],
        dim_heads=[16, 24, 20, 32], act_layers=act_layers, act_layer2=act_layer,
        **kwargs)
    return model
