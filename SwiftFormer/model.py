...
class ConvEncoder(nn.Module):
    ...
    def __init__(self, dim, hidden_dim=64, kernel_size=3, drop_path=0., act_layer=nn.GELU, use_layer_scale=True):
        super().__init__()
...
class SwiftFormerLocalRepresentation(nn.Module):
    ...
    def __init__(self, dim, kernel_size=3, drop_path=0., act_layer=nn.GELU, use_layer_scale=True):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=kernel_size, padding=kernel_size // 2, groups=dim)
        self.norm = nn.BatchNorm2d(dim)
