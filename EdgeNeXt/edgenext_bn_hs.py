...
class EdgeNeXtBNHS(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 use_pos_embd_global=False, d2_scales=[2, 3, 4, 5], act_layer=nn.GELU, **kwargs):
        super().__init__()
            ...
            for j in range(depths[i]):
                if j > depths[i] - global_block[i] - 1:
                    if global_block_type[i] == 'SDTA_BN_HS':
                        stage_blocks.append(SDTAEncoderBNHS(dim=dims[i], act_layer=act_layer, drop_path=dp_rates[cur + j],
                    ...
                    stage_blocks.append(ConvEncoderBNHS(dim=dims[i], act_layer=act_layer, drop_path=dp_rates[cur + j],
